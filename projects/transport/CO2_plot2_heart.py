...
def abrazol0D(i, cstart, cend, ax, con):
    data = pd.read_csv(os.path.join("results", "Bathsheba_CO2_pul", "heart_kim_lit", concentration_labels[con], 'pul_cap' + ".txt"), header=None)
    t = data[0]
    var_s = data[1]
    var_e = data[2]
...
plt.tight_layout()
plt.savefig("PlasmaCO2_heart.jpg", dpi=150)
