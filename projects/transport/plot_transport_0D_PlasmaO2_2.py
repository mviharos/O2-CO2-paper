...
def abrazol(i, cstart, cend):
    data = pd.read_csv(os.path.join("results",'Bathsheba', models[i] , elements[i] + ".txt"),header=None)
    t = data[0]
    RBCs = data[17]
    RBCe = data[18]
...
def abrazol0D(i, cstart, cend):
    data = pd.read_csv(os.path.join("results",'Bathsheba', "p10", "C_Plasma_O2", compartments[i] + ".txt"),header=None)
    t = data[0]
    RBCs = data[1]
    RBCe = data[2]
...
plt.tight_layout()
plt.savefig("PlasmaO2.jpg", dpi=150)
